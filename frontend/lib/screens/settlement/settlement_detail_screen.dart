...
class _SettlementDetailScreenState extends State<SettlementDetailScreen> {
  ...
  Color _primaryText(BuildContext context) =>
      _isDark(context) ? AppTheme.textPrimary : Colors.black87;
  Color _secondaryText(BuildContext context) =>
      _isDark(context) ? AppTheme.textSecondary : Colors.black54;
    ...
    if (expenses.isNotEmpty) {
      actions.add(
        SettlementActionButton(
          onPressed: () => _showAllItemsPreview(expenses, s),
  ...
  Widget build(BuildContext context) {
    ...
    final isSingleSettlement =
        (s?['settlement_type'] ?? 'single').toString().toLowerCase() ==
        'single';
  ...
  void _showItemPreview(Map<String, dynamic> item) {
    ...
    showItemDetailPreviewDialog(
      context: context,
      item: {...item, 'batch_notes': prov.currentSettlement?['notes']},
      title: 'Rincian Item Settlement',
    ...
    showDialog(
      ...
      builder: (ctx) {
        ...
        return AlertDialog(
          ...
          titlePadding: const EdgeInsets.fromLTRB(20, 20, 8, 10),
          title: Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Padding(
                padding: EdgeInsets.only(top: 2),
                child: Icon(Icons.list_alt_rounded, color: AppTheme.accent),
              ),
              ...
              Expanded(
                child: Builder(
                  builder: (context) {
                    ...
                    final type = (parent['settlement_type'] ?? 'batch')
                        .toString()
                        .toLowerCase();
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            const SizedBox(width: 10),
            const Expanded(
              child: Text('Riwayat Komentar', overflow: TextOverflow.ellipsis),
            ),
            IconButton(
              onPressed: () => Navigator.pop(ctx),
              icon: const Icon(Icons.close_rounded),
  ...
  Widget _buildActionCell(
    ...
    final previewBtn = IconButton(
      icon: Icon(Icons.info_outline_rounded, color: AppTheme.primary, size: 20),
      tooltip: 'Pratinjau Rincian',
      onPressed: () => _showItemPreview(exp),
    );
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        children: [
                          const SizedBox(
                            height: 10,
                          ), // Memberi ruang agar label tidak menempel ke judul
                                    ...
                                    child: Column(
                                      children: children.map((c) {
                                        ...
                                        final isPending =
                                            c['status'] == 'pending';
                                        final isSelected =
                                            selectedSubCategoryIds.contains(
                                              subId,
                                            );
                          ...
                          OutlinedButton.icon(
                            ...
                            label: Text(
                              selectedFileName ?? 'Upload Bukti (opsional)',
                            ),
                          ),
                ...
                onPressed: _isSaving
                    ...
                    : () async {
                        ...
                        try {
                          ...
                          final expenses =
                              currentSettlement?['expenses'] as List? ?? [];
                          ...
                          if (isFirstExpense) {
                            ...
                            success = await prov
                                .saveFirstExpenseAndCommitSettlement(
  ...
  Future<void> _approveExpense(int expenseId, String action) async {
    ...
    if (action == 'reject') {
                        ...
                        child: Column(
                          children: controllers.asMap().entries.map((entry) {
                            ...
                            return Padding(
                              key: ValueKey(
                                ctrl,
                              ), // Use controller as key to prevent reuse issues
                              padding: const EdgeInsets.only(bottom: 8),
                              child: Row(
                                children: [
                                  const Icon(
                                  ...
                                  if (controllers.length > 1)
                                    IconButton(
                                      ...
                                      onPressed: () {
                                        ...
                                        // Dispose after the rebuild has potentially finished
                                        Future.delayed(
      ...
      // Dispose all controllers after dialog is fully closed
      ...
      if (confirmReject != true || reasonsList.isEmpty) return;
      notes = jsonEncode(reasonsList);
    }
    ...
    if (mounted) {
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: Column(
              ...
              children: [
                Text(
                  prov.error ??
                      'Kategori item ini belum disetujui oleh manager.',
              ...
              ElevatedButton.icon(
                onPressed: () {
                  ...
                  Navigator.push(
                    ...
                    if (mounted) {
                      context.read<SettlementProvider>().loadSettlement(
                        widget.settlementId,
                      );
                    }
                  });
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(success ? 'Expense $label ✓' : (prov.error ?? 'Gagal')),
  ...
  Future<void> _bulkDeleteExpenses() async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Hapus $count pengeluaran yang dipilih? Tindakan ini tidak bisa dibatalkan.',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
    ...
    if (!mounted) return;
    final prov = context.read<SettlementProvider>();
    final success = await prov.bulkDeleteExpenses(
      _selectedExpenses.toList(),
      widget.settlementId,
    );
    if (success) {
  ...
  Future<void> _rejectSettlement() async {
    ...
    if (result != null && result.isNotEmpty) {
      ...
      if (mounted) {
        if (success) {
          AppSnackbar.success(
            'Settlement berhasil ditolak dan dikembalikan ke draft ✓',
          );
          prov.loadSettlement(widget.settlementId);
        } else {
                ...
                child: Column(
                  ...
                  children: [
                      ...
                      Padding(
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                'Bukti saat ini: ${exp['evidence_filename']}',
                                style: const TextStyle(
                                  fontSize: 11,
                                  color: AppTheme.textSecondary,
                                ),
                    ...
                    OutlinedButton.icon(
                      ...
                      icon: const Icon(Icons.upload_file_rounded),
                      label: Text(
                        selectedFileName ?? 'Ganti / Tambah Bukti (opsional)',
                      ),
                    ),
                    if (selectedFileName != null)
                      Padding(
          ...
          return AlertDialog(
            ...
            content: SizedBox(
              ...
              child: ConstrainedBox(
                ...
                child: Builder(
                  builder: (ctx2) {
                    final checklistCtrl = ScrollController();
                    return AppScrollbar(
  ...
  Widget _buildSaveChecklistButton(
    ...
    return ElevatedButton(
      ...
      onPressed: () async {
        ...
        final success = await prov.updateExpensePartial(
          expenseId,
          widget.settlementId,
          {'notes': jsonEncode(list), 'status': newStatus},
        );
