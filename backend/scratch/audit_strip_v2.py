...
def run_comprehensive_audit_v2():
    try:
        ...
        print("[*] Memulai audit menyeluruh v2...")
        ...
        md_content = "# Audit Detail Item Strip v2\n\n"
        md_content += "Laporan ini membedakan antara item yang menggunakan subkategori `-` (seperti A0, B0) dengan item yang langsung menggunakan Kategori Induk.\n\n"
        
        grand_total = sum(r[4] for r in rows)
        md_content += f"### GRAND TOTAL: Rp {grand_total:,.0f}\n\n"

        md_content += "| Kategori Induk | Subkategori | Kode | Status | Deskripsi | Amount | Settlement |\n"
        md_content += "| :--- | :--- | :--- | :--- | :--- | :--- | :--- |\n"
        
        for r in rows:
            # r: (nama_induk, nama_sub, kode_sub, desc, amount, date, settlement, status)
            # Jika Induk Langsung, nama_induk adalah '---' di query, tapi kita mau tampilkan c.name
            parent = r[0]
            sub = r[1]
            ...
            md_content += f"| {parent} | {sub} | {r[2]} | {r[7]} | {r[3]} | Rp {r[4]:,.0f} | {r[6]} |\n"

        output_file = "LAPORAN_STRIP_DETAIL_V2.md"
        with open(output_file, "w", encoding="utf-8") as f:
            f.write(md_content)

        print(f"[+] Laporan v2 selesai: {output_file}")
        conn.close()
    except Exception as e:
        print(f"[!] Error: {e}")

if __name__ == "__main__":
    run_comprehensive_audit_v2()
